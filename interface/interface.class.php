...
class WPLeadsInterface{
	...
	static function processSync($login,$email,$errors){
	...
	static function integrationSet(){
	...
	static function registrationFormActivated(){
		$setup=WPLeadsInterface::get_wp_settings();
		if($setup["activated"]) return true;
		return false;
	}
	static function commentFormActivated(){
		$setup=WPLeadsInterface::get_wp_settings();
		if($setup["comment_activated"]) return true;
		return false;
	}
	static function deactivateRegistrationForm(){
		WPLeadsInterface::set_wpleads_setup_registration_activated(0);
	}
	static function getConfiguredText($bool){
		if($bool) return "Yes";
		return "No";
	}
	static function fullTrim($post){
		if(is_array($post)){
	...
	static function sanitizeData($post){
		if(is_array($post)){
			foreach($post as $key=>$value){
				if(!is_array($value)){
	...
	*/
	...
	static function set_wpleads_setup_commentform_text($text){
